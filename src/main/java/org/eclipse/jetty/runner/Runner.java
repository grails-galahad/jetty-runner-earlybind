...
public class Runner
{
    ...
    protected Resource _warCtx;
    protected String _warCtxPath;
        ...
        for (int i=0;i<args.length;i++)
        {
            ...
            else // process contexts
            {
                ...
                try(Resource ctx = Resource.newResource(args[i]);)
                {
                    ...
                    {
                        // assume it is a WAR file, cache values for init after server start
						_warCtx = ctx;
						_warCtxPath = contextPath;
                    }
                }
                //reset
    ...
    private void initWebContext() throws Exception {
        WebAppContext webapp = new WebAppContext(_warCtx.toString(),_warCtxPath);
